...
export default function  Milestone({ value,status}) {
    ...
    const [milestoneStatus, setMilestoneStatus] = useState(status);

    const handlePress = () => {
        setMilestoneStatus(true);
        // Find the corresponding milestone object in the twoMonthMilestones array
        const milestoneCategory = Object.keys(twoMonthMilestones).find(
            (category) =>
                twoMonthMilestones[category].find((milestone) => milestone.value === value)
        );
        const milestoneIndex = twoMonthMilestones[milestoneCategory].findIndex(
            (milestone) => milestone.value === value
        );
        // Update the status property of the milestone object
        twoMonthMilestones[milestoneCategory][milestoneIndex].status = true;
    };
